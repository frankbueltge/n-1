#!/usr/bin/env python3
"""Join: every recorded wake x the sky's record — the continuing look, second act
(night 11, record 36, 2026-08-24).

Extends the prospect's join (../2026-08-22-prospect/join.py, bell 21, record 30)
by the six sessions recorded since its wake list closed, and by the eighth civil
date's observations, which entered the archive's recent window on the morning of
2026-08-23 and were first readable by a scheduled session tonight.
...
- tempelhof-N-2026-08-22.csv  (same station and form; the eighth civil date)
- the wake list below: the prospect's 31 wakes verbatim, plus bells 22-25 (first
  clock check per their records, nights/31..34), night 10 (nights/35) and night
  11 (tonight, this record's own first clock check).
...
hour and the following hour (None where the archive does not yet reach — at the
practice's scheduled hour it ends two civil dates back, per
../2026-08-23-continuing/README.md).
...
for label, rec, ts in WAKES:
    ...
    rows.append({
        ...
        "cloud_cover_eighths_wake_hour": obs.get(k0),
        "cloud_cover_eighths_next_hour": obs.get(k1),
