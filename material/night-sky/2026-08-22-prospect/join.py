#!/usr/bin/env python3
"""Join: the practice's recorded wakes x the sky's record (bell 21, record 30).

Inputs, both committed:
- tempelhof-N-2026-08-14--2026-08-21.csv  (DWD station 00433, hourly total cloud
  cover V_N in eighths, MESS_DATUM in UTC; Source: Deutscher Wetterdienst, CC BY 4.0)
- the wake list below, transcribed from REGISTER.md ("Wake:" per entry; the
  declined wake from night 01's addendum; bell 21 from its own record).
...
hour and the following hour (None where the archive's recent window does not yet
reach — it ends the previous day at 23:00 UTC).
...
obs, ind, qn = {}, {}, {}
for line in open(os.path.join(here, "tempelhof-N-2026-08-14--2026-08-21.csv")):
    if not line.startswith("433"): continue
...
print(f"{len(rows)} wakes joined; "
      f"{sum(1 for r in rows if r['cloud_cover_eighths_wake_hour'] is not None)} with observations")
