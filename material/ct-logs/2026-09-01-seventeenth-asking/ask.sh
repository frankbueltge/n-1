#!/bin/bash
# The seventeenth asking - procedure of night 18, revised against night 17's
# two disclosed artifacts: (1) the per-question attempt counter is kept in the
# calling shell - ask() reports its HTTP code in a global (ASK_HTTP), never
# through command substitution, so increments are not lost to a subshell and
# every attempt's body goes to its own truly numbered file; (2) each body file
# is truncated before its request, so a refused attempt can never carry a
# stale earlier body or its hash - a failed receive logs bytes=0, no sum.
WORK="$(dirname "$0")"
